...
tempData <- mice(t,m=1,maxit=50,meth='pmm',seed=500)
...
```{r demography of adjucts}
CLUSTER<-Clust$cluster
df2<-cbind(df,CLUSTER)
options(digits=2)
DemVars<-df2[DEMGVARS]
list<-do.call(rbind, sapply(DemVars, function(x){
    if(is.numeric(x)==TRUE){
      means<-aggregate(x ~ df2$CLUSTER, FUN=mean, na.action=na.omit)
      print(means)[,2]
      }
    else{
      prop.table(table(x, df2$CLUSTER),2)
      }
  }))
...
```{r Institutions_of_adjucts}
CLUSTER<-Clust$cluster
df3<-cbind(df,CLUSTER)
options(digits=2)
InstVars<-df3[INSTVARS]
list<-do.call(rbind, sapply(InstVars, function(x){
    if(is.numeric(x)==TRUE){
      means<-aggregate(x ~ df3$CLUSTER, FUN=mean, na.action=na.omit)
      print(means)[,2]
      }
    else{
      prop.table(table(x, df3$CLUSTER),2)
      }
  }))
